...
const Profile = () => {
  ...
  const fetchUser=async()=>{
    try {
        const res=await axios.post("https://charity-vercel-4ebc.vercel.app/auth/getUser",{userId:parseInt(userId)},{headers:{"Authorization":userToken}})
        setUser(res.data)
  ...
  const fetchUserData=async()=>{
    try {
        const res=await axios.post("https://charity-vercel-4ebc.vercel.app/campaigns/getUserData",{userId:parseInt(userId)},{headers:{"Authorization":userToken}})
  ...
  const handlePasswordChange = async() => {
    try {
        const res=await axios.post("https://charity-vercel-4ebc.vercel.app/auth/changePassword",{userId:parseInt(userId),oldPassword:oldPassword,newPassword:newPassword},{headers:{"Authorization":userToken}})
    if(res.status<400){
        alert(res.data)
        setOldPassword("")
        setNewPassword("")
