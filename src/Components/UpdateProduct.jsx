...
const ProductCard = ({ product,setTrigger,trigger }) => {
 ...
 const handleUpdate=async()=>{
    try {
        const res=await axios.post("http://localhost:5500/products/update",{prodId:parseInt(product.id),quantity:parseInt(currentQuantity)},{headers:{"Authorization":userToken}})
        setIsEditing(false)
        if(res.status===200){
            setTrigger(!trigger)
        }
